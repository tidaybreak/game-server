...
const int Gang_Boss_Revive_Move_Able = 21;
const int gang_boss_act_icon_id = 19;

Gang_Boss_Scene::Gang_Boss_Scene(void){
	reset();
}
...
void Gang_Boss_Scene::reset(void){
	Move_Scene::reset();
	logic_uuid_ = 0;
	act_result_ = -1;
	act_time_type_ = 0;
...
void Gang_Boss_Scene::set_logic_uuid(const int64_t uuid){
	logic_uuid_ = uuid;
}

void Gang_Boss_Scene::npc_init(int scene_id, int sort, int sub_sort){
	if(!world_boss_manager()){
		set_scene_status(SCENE_RECOVER_STATUS);
	...
	if(boss_info){
		...
		if(it != npc_section_.record_map.end()){
			if(it->second.config.npc_vec.empty() == false){
				...
				drop_box_coord_ = it->second.config.birth_coord;
			}
		}
	}
...
int Gang_Boss_Scene::tick(const Time_Value &now){
	...
	if(this->scene_status_ == SCENE_NORMAL_STATUS){
		if(this->act_end_time_.sec() - now.sec() <= act_fight_time_){
			...
			change_move_value(Gang_Boss_Revive_Move_Able, true);
			if(act_time_type_ == 0){
				act_time_type_ = 1;
				broadcast_act_time();
			}
	...
	switch(act_state_){
		...
		case 2:{
			if(now >= balance_time_){
				act_over();
				act_state_ = 3;
				if(act_result_ == 0){// lose
					balance_time_ = now + Time_Value(0);
				}else{// win keep 5 min
					balance_time_ = now + Time_Value(5*60);
					act_end_time_ = balance_time_;
					act_time_type_ = 2;
					broadcast_act_time();
					announce_to_guild(create_scene_args_.id, 7111, 0, "", "5");
					refresh_gang_boss_drop_box();
				}
			}
			break;
		}
...
int Gang_Boss_Scene::enter_scene(Mover *mover){
	...
	if(mover && mover->player_self()){
		...
		if(player_info){
			...
			for(int i = 0; i < player_info->ability_2.val_1; ++i){
				mover->player_self()->world_boss_add_power_up_buf(gang_boss_act_icon_id);
			}
		}
		for(int i = 0; i < world_boss_manager()->boss_info_.bonus_id; ++i){
			mover->player_self()->world_boss_add_leader_buf(gang_boss_act_icon_id);
		}
	}
	return 0;
}

int Gang_Boss_Scene::exit_scene(Mover *mover, bool offline){
	...
	if(mover && mover->player_self()){
		...
		mover->player_self()->world_boss_clear_power_up_buf(gang_boss_act_icon_id);
		mover->player_self()->world_boss_clear_leader_buf(gang_boss_act_icon_id);
	}
	return 0;
}

int Gang_Boss_Scene::init_scene_normal(int scene_id, int scene_order, int map_id/* = 0*/){
	...
	is_check_boss_step_ = CONFIG_CACHE_WORLD_BOSS->get_boss_weak_check(gang_boss_act_icon_id);
	boss_step_1_value_ = CONFIG_CACHE_WORLD_BOSS->get_boss_weak_1(gang_boss_act_icon_id);
	boss_step_2_value_ = CONFIG_CACHE_WORLD_BOSS->get_boss_weak_2(gang_boss_act_icon_id);
	boss_step_3_value_ = CONFIG_CACHE_WORLD_BOSS->get_boss_weak_3(gang_boss_act_icon_id);
	return 0;
}

int Gang_Boss_Scene::client_scene_complete(Fighter &fighter) {
	...
	msg_act_info.icon_id = gang_boss_act_icon_id;
   ...
   ability_num = player_info->ability_1.val_2 - player_info->ability_1.val_1;
	...
	msg_rank_info.icon_id = gang_boss_act_icon_id;
...
int Gang_Boss_Scene::update_move_able_info(Mover* mover){
		...
		if((*it).move_value.empty() == false){
			...
			for(s_it = (*it).move_value.begin(); s_it != (*it).move_value.end(); ++s_it){
				if(s_it->val_2 == MOVE_ABLE_FROM){
					if(s_it->val_1 == Gang_Boss_Revive_Move_Able){
...
void Gang_Boss_Scene::hook_player_enter_battle(Scene_Player* player){
	if(!player){
		return;
	}
	if(!world_boss_manager()){
	...
	if(first_blood){
		...
		announce_to_guild(create_scene_args_.id, 7101, player->role_id(), player->role_name());
	}
}

void Gang_Boss_Scene::hook_battle_finish(int battle_type, int win_pos, std::vector<Mover*> win_mover_vec, std::vector<Mover*> lose_mover_vec, std::vector<int> kill_monsters){
	if(!world_boss_manager()){
		return;
	}
	if(win_pos == 0){
		...
		int revive_time = CONFIG_CACHE_WORLD_BOSS->get_revive_time(gang_boss_act_icon_id);
...
void Gang_Boss_Scene::be_hurt_and_sub_blood(Battle_Scene* battle_scene, double sub_blood, double current_blood, role_id_t attacker_id, int point_id){
	if(!battle_scene){
		return;
	}
	if(!world_boss_manager()){
	...
	if(world_boss_manager()->boss_info_.hp_cur <= 0){
		...
		int rapid_kill_time = CONFIG_CACHE_WORLD_BOSS->get_boss_rapid_kill_time(gang_boss_act_icon_id);
		...
		if(player_info){//
			...
			announce_to_guild(create_scene_args_.id, 7102, player_info->rank_info.role_id,
					player_info->rank_info.role_name);
			// 击杀boss通知
			{
				Block_Buffer kb_buf;
				kb_buf.make_message(INNER_ACT_SCENE_SYNC_INFO);
				kb_buf.write_int32(gang_boss_act_icon_id);
				kb_buf.write_int8(4);
		...
		if(bonus_id != 0){
			add_leader_buf();
			announce_to_guild(this->create_scene_args_.id, 7103, bonus_role_id, bonus_role_name);
		}
	}
	world_boss_hp_change_ = true;
}

void Gang_Boss_Scene::set_world_boss_blood(const int blood){
...
int Gang_Boss_Scene::get_world_boss_act_time_type(void){
	return act_time_type_;
}

void Gang_Boss_Scene::kick_player(const role_id_t role_id){
	...
	// 恢复进入的血量
	double src_hp = player->blood_max();
	player->restore_hp(src_hp, false);
	int dest = 0;
	player->inner_transmit(dest,0,0);
}

void Gang_Boss_Scene::kick_all_player(void){
	Scene_Layer *layer = scene_layer(0);
	if (layer) {
...
void Gang_Boss_Scene::act_over(void){
	if(!world_boss_manager()){
		return;
	}
	world_boss_manager()->refresh_rank();
	//奖励
	MSG_81000602 msg;
	msg.icon_id = gang_boss_act_icon_id;
	...
	reward_buf.write_int32(gang_boss_act_icon_id);
	...
	const World_Boss_Level_Config* cfg_lv = CONFIG_CACHE_WORLD_BOSS->get_level_config_from_lv(cfg_boss_lv, gang_boss_act_icon_id);
	if(cfg_lv){
...
//			}
		...
		//rank
		const World_Boss_Reward_Config* cfg_rank = CONFIG_CACHE_WORLD_BOSS->get_reward_config(cfg_boss_lv, self_rank, gang_boss_act_icon_id);
		if(cfg_rank){
			{// 资源奖励
		...
		if(mover){
			...
			if(player){
				...
				msg.rank = player_it->second.rank_info.rank;
	...
	{// act_end
		...
		end_buf.write_int32(gang_boss_act_icon_id);
		end_buf.write_int64(create_scene_args_.id);
	...
	if(first_role_id_1 != 0 || first_role_id_2 != 0 || first_role_id_3 != 0){
		...
		const World_Boss_Reward_Config* first_rank = CONFIG_CACHE_WORLD_BOSS->get_reward_config(cfg_boss_lv, 1, gang_boss_act_icon_id);
		if(first_rank){
		...
		if(first_role_id_1){
			announce_to_guild(create_scene_args_.id, 7104, first_role_id_1, first_role_name_1,
					ostr.str());
		}
		if(first_role_id_2){
			announce_to_guild(create_scene_args_.id, 7114, first_role_id_2, first_role_name_2,
					ostr.str());
		}
		if(first_role_id_3){
			announce_to_guild(create_scene_args_.id, 7115, first_role_id_3, first_role_name_3,
					ostr.str());
		}

	}else{
		announce_to_guild(create_scene_args_.id, 7105, 0, "");
	}
	//
}

void Gang_Boss_Scene::act_recover(void){
	kick_all_player();
	...
	world_boss_manager()->set_icon_id(gang_boss_act_icon_id);
}

void Gang_Boss_Scene::broadcast_boss_info(void){
	if(!world_boss_manager()){
		return;
	}
	int blood = world_boss_manager()->boss_info_.hp_cur;
...
void Gang_Boss_Scene::broadcast_rank_info(void){
	if(!world_boss_manager()){
		return;
	}
	MSG_81000604 msg;
...
void Gang_Boss_Scene::broadcast_act_time(void){
	MSG_81000605 msg;
	msg.icon_id = gang_boss_act_icon_id;
...
void Gang_Boss_Scene::add_leader_buf(void){
	Scene_Layer *layer = scene_layer(0);
	if (layer) {
		for (Mover_Map::iterator it = layer->mover_map_.begin();
				it != layer->mover_map_.end(); ++it) {
			if (it->second && it->second->player_self()) {
				it->second->player_self()->world_boss_add_leader_buf(gang_boss_act_icon_id);
			}
		}
	}
	broadcast_boss_info();
...
void Gang_Boss_Scene::add_first_blood_reward(const role_id_t role_id){
	if(!world_boss_manager()){
		return;
	}
	int cfg_boss_lv = world_boss_manager()->boss_info_.cfg_boss_lv;
...
void Gang_Boss_Scene::add_fight_reward(const role_id_t role_id, int damage, const int add_effect){
	if(!world_boss_manager()){
		return;
	}
	if(damage <= 0){
...
void Gang_Boss_Scene::force_end_battle(void){
	Scene_Layer *layer = scene_layer(0);
	if (layer) {
		for (Mover_Map::iterator it = layer->mover_map_.begin();
				it != layer->mover_map_.end(); ++it) {
...
World_Boss_Manager* Gang_Boss_Scene::world_boss_manager(void){
	return GANG_BOSS_MANAGER->world_boss_manager(create_scene_args_.id);
}

void Gang_Boss_Scene::refresh_gang_boss_drop_box(void){
	World_Boss_Manager* wb_manager = world_boss_manager();
	if(!wb_manager){
		return;
	}
	int cfg_boss_lv = wb_manager->boss_info_.cfg_boss_lv;
	const World_Boss_Level_Config* cfg_lv = CONFIG_CACHE_WORLD_BOSS->get_level_config_from_lv(cfg_boss_lv, gang_boss_act_icon_id);
	if(!cfg_lv){
		return;
	}
	if(cfg_lv->drop_box.empty()){
		return;
	}
	int drops_id = cfg_lv->drop_box[0];
	const Drop_Config* drop_config = CONFIG_CACHE_NPC->find_drop_config_cache(drops_id);
	if (!drop_config){
		return;
	}
	Drop_Vec drops_all;
	NPC_MANAGER->process_drop_mode(0, drops_all, drop_config, 1, 1, 1, 1);
	World_Boss_Player_Info_Map::iterator it;
	for(it = wb_manager->player_info_map_.begin(); it != wb_manager->player_info_map_.end(); ++it){
		NPC_MANAGER->create_drops(drop_config->name, Drops::DROP_TYPE_BOX, drops_id, drop_config->jid,
				drops_all, this, NULL, drop_box_coord_, it->second.rank_info.role_id, 0, 0);
	}
}

void Gang_Boss_Scene::check_weak_up_buf(void){
	...
	if(step_value < boss_step_3_value_){// 40%
		if(world_boss_manager()->boss_info_.buffer_step == 0){
			...
			clear_weak_up_1_buf();
			world_boss_manager()->boss_info_.buffer_step = 3;
			announce_to_guild(create_scene_args_.id, 7010, 0, "");
			broadcast_boss_info();
		...
		if(world_boss_manager()->boss_info_.buffer_step == 0){
			...
			announce_to_guild(create_scene_args_.id, 7009, 0, "");
			broadcast_boss_info();
		}else if(world_boss_manager()->boss_info_.buffer_step == 1){// HP低于80%时，BOSS物免降低20%
			clear_weak_up_1_buf();
			add_weak_up_2_buf();
		...
		if(world_boss_manager()->boss_info_.buffer_step == 0){
			...
			announce_to_guild(create_scene_args_.id, 7008, 0, "");
			broadcast_boss_info();
		}else if(world_boss_manager()->boss_info_.buffer_step == 1){// HP低于80%时，BOSS物免降低20%
...
void Gang_Boss_Scene::add_weak_up_1_buf(void){
	const World_Boss_Base_Config* cfg_base = CONFIG_CACHE_WORLD_BOSS->get_base_config(gang_boss_act_icon_id);
	if(!cfg_base){
		return;
	}
...
void Gang_Boss_Scene::clear_weak_up_1_buf(void){
	const World_Boss_Base_Config* cfg_base = CONFIG_CACHE_WORLD_BOSS->get_base_config(gang_boss_act_icon_id);
	if(!cfg_base){
		return;
	}
...
void Gang_Boss_Scene::add_weak_up_2_buf(void){
	const World_Boss_Base_Config* cfg_base = CONFIG_CACHE_WORLD_BOSS->get_base_config(gang_boss_act_icon_id);
	if(!cfg_base){
		return;
	}
...
void Gang_Boss_Scene::clear_weak_up_2_buf(void){
	const World_Boss_Base_Config* cfg_base = CONFIG_CACHE_WORLD_BOSS->get_base_config(gang_boss_act_icon_id);
	if(!cfg_base){
		return;
	}
...
void Gang_Boss_Scene::add_weak_up_1_buf_to_fighter(Fighter* fighter){
	if(!fighter){
		return;
	}
	const World_Boss_Base_Config* cfg_base = CONFIG_CACHE_WORLD_BOSS->get_base_config(gang_boss_act_icon_id);
...
void Gang_Boss_Scene::add_weak_up_2_buf_to_fighter(Fighter* fighter){
	if(!fighter){
		return;
	}
	const World_Boss_Base_Config* cfg_base = CONFIG_CACHE_WORLD_BOSS->get_base_config(gang_boss_act_icon_id);
...
void Gang_Boss_Scene::announce_to_all(const int ann_id, const role_id_t role_id, std::string role_name, std::string str1/* = ""*/, std::string str2/* = ""*/, std::string str3/* = ""*/, std::string str4/* = ""*/){
...
void Gang_Boss_Scene::announce_to_scene(const int ann_id, const role_id_t role_id, std::string role_name, std::string str1/* = ""*/, std::string str2/* = ""*/, std::string str3/* = ""*/, std::string str4/* = ""*/){
	std::vector<Chat_Content> ann_contents;
	Role_Info role_info;
	Chat_Content new_content;
	if(role_id != 0){
...
void Gang_Boss_Scene::announce_to_guild(const int64_t gang_id, const int ann_id, const role_id_t role_id, std::string role_name, std::string str1/* = ""*/, std::string str2/* = ""*/, std::string str3/* = ""*/, std::string str4/* = ""*/){
	std::vector<Chat_Content> ann_contents;
	Role_Info role_info;
	Chat_Content new_content;
	if(role_id != 0){
