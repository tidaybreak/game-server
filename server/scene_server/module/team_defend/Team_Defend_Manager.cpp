...
#include "Team_Defend_Manager.h"
#include "Team_Defend_Scene.h"
#include "Scene_Monitor.h"
#include "Scene_Player.h"

Team_Defend_Manager::Team_Defend_Manager() {
...
int Team_Defend_Manager::init(const Monitor_Unique_ID &scene_uid) {
...
int Team_Defend_Manager::single_scene_status(void) {
	LOG_TEXT("team scene size:%d", scene_map_.size());

	return 0;
}

int Team_Defend_Manager::tick(Time_Value &now) {
	...
	for (Muid_Role_Scene_Map::iterator muid_it = scene_map_.begin(); muid_it != scene_map_.end(); ++muid_it) {
		for (Role_Scene_Map::iterator scene_it = muid_it->second.begin(); scene_it != muid_it->second.end(); ) {
			if(scene_it->second->is_recover_status() && scene_it->second->is_recover_time(now)){
				...
				scene_pool_.push((Team_Defend_Scene*)scene);
...
void Team_Defend_Manager::show_scene_status(void) {
...
Move_Scene *Team_Defend_Manager::find_scene(const Monitor_Unique_ID &muid, const role_id_t role_id, const Create_Scene_Args &csa) {
	...
	if (muid_it_find_it != scene_map_.end()) {
		...
		if (scene_it_find_it != muid_it_find_it->second.end()) {
			...
			Team_Defend_Scene *scene = scene_pool_.pop();
...
Move_Scene *Team_Defend_Manager::find_scene_simple(const Monitor_Unique_ID &muid, const role_id_t role_id, const Create_Scene_Args &csa) {
