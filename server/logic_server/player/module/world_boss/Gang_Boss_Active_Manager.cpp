...
#include "Gang_Boss_Active_Manager.h"
...
Gang_Boss_Active_Manager::Gang_Boss_Active_Manager(void):first_start_(false), act_id_(0),
		act_open_(false), icon_open_(false){

}

Gang_Boss_Active_Manager::~Gang_Boss_Active_Manager(void){

}

int Gang_Boss_Active_Manager::start(void){
	load_active_info();
...
int Gang_Boss_Active_Manager::tick(const Time_Value& now){
	return 0;
	if(first_start_){
		first_start_ = false;
	...
	if(active_status_.status != 0 &&
		...
		for(Logic_Monitor::Logic_Player_Role_Id_Map::const_iterator it = LOGIC_MONITOR->player_role_id_map().begin();
			...
			if(it->second){
				it->second->send_act_scene_to_client(gang_boss_act_icon_id);
			}
		}
		//activate to scene
...
void Gang_Boss_Active_Manager::daily_refresh(void){

}

int Gang_Boss_Active_Manager::get_enter_level_limit(void){
	return CONFIG_CACHE_WORLD_BOSS->get_enter_lv_limit(gang_boss_act_icon_id);
}

void Gang_Boss_Active_Manager::get_active_info(int& status, int& remain_time, int &party_id){
	if(icon_open_){
		status = 0;
		remain_time = 0;
...
int Gang_Boss_Active_Manager::cmd_sync_active_info(Block_Buffer& buf){
	return 0;
}

int Gang_Boss_Active_Manager::load_active_info(void){
	active_info_.reset();
	MONGO_OPERATOR->load_world_boss_detail(active_info_);
	return 0;
}

int Gang_Boss_Active_Manager::active_ready(void){
	//准备世界公告
	//system_announce(ANNOUNCE_DISTRICT_SYSTEM, ANNOUNCE_ID_BONFIRE_READY);
	return 0;
}
int Gang_Boss_Active_Manager::active_start(void){
	Time_Value now = Time_Value::gettimeofday();
	...
	buf.write_int32(gang_boss_act_icon_id);
	buf.write_int64(0);
	...
	// 开启公告
	system_announce(ANNOUNCE_DISTRICT_GANG, 7107);
	act_id_ = now.sec();
	return 0;
}

int Gang_Boss_Active_Manager::active_finish(void){
	// 结束世界公告
	//system_announce(ANNOUNCE_DISTRICT_SYSTEM, ANNOUNCE_ID_BONFIRE_FINISH);
	return 0;
}

void Gang_Boss_Active_Manager::cal_active_time(const Time_Value& now, Active_Status& active_status){
	tm tm1;
	time_t t1 = now.sec();
	localtime_r(&t1, &tm1);
...
void Gang_Boss_Active_Manager::system_announce(const int chat_type, const int ann_id, const gang_id_t gang_id/* = 0*/, std::string str1/* = ""*/, std::string str2/* = ""*/, std::string str3/* = ""*/){
...
void Gang_Boss_Active_Manager::resource_to_item(std::vector<Money_Add_Info> money_list, std::vector<Item_Detail>& item_vec){
	int pack_num = 5000;
	int item_amount = 0;
	int item_id = 0;
...
int Gang_Boss_Active_Manager::cmd_start_active(int time){
...
int Gang_Boss_Active_Manager::cmd_finish_active(void){
	if(active_status_.status != 3){//
		return -1;
	}
	int remain_time = 2;
...
bool Gang_Boss_Active_Manager::is_open(void){
	return act_open_;
}

void Gang_Boss_Active_Manager::listen_open_active(Time_Value before_start_time, Time_Value start_time, Time_Value end_time, const role_id_t gang_id){
	...
	active_info_.serialize(buf);
	buf.finish_message();
	int dest_scene = 80210001;
	LOGIC_MONITOR->send_to_scene(dest_scene, 0, buf);
	// 开启世界公告
	system_announce(ANNOUNCE_DISTRICT_GANG, 7107,gang_id);
	act_open_ = true;
}

void Gang_Boss_Active_Manager::listen_close_active(void){
	act_open_ = false;
}

void Gang_Boss_Active_Manager::listen_open_active_icon(Time_Value before_start_time, Time_Value start_time, Time_Value end_time){
	icon_open_ = true;
...
void Gang_Boss_Active_Manager::listen_close_active_icon(void){
	icon_open_ = false;
}

void Gang_Boss_Active_Manager::gang_boss_open(const role_id_t gang_id, const int fight_time, const int remain_time, const int kill_num){
	Block_Buffer buf;
	buf.make_message(INNER_ACT_SCENE_START);
	buf.write_int32(gang_boss_act_icon_id);
	buf.write_int64(gang_id);
...
int Gang_Boss_Active_Manager::req_enter_scene(Logic_Player* player){
	if(!player){
		return 0;
	}
	...
	int state = 0;
	int remain_time = 0;
	GANG_MANAGER->get_gang_boss_act_state(gang_id, state, remain_time);
	if (state != 3) {
		return ERROR_BONFIRE_NO_START;
	}
	bool can_join = GANG_MANAGER->can_join_gang_boss(player->role_id(), player->gang_id());
	if(!can_join){
		return ERROR_GANG_WAR_ALREADY_ATTEND;
	}
	int dest_scene = 80210001;
	Coord coord(0, 0, 1);
	int64_t arg3 = gang_id;
	std::string arg4 = "";
	// sync to scene
	Block_Buffer buf;
	...
	GANG_MANAGER->set_gang_boss_join_flag(player->role_id(), player->gang_id());
	// enter scene
	int transmit_result = player->inner_transmit(dest_scene, 0, coord, arg3, arg4);
...
int Gang_Boss_Active_Manager::inner_sync_info(Logic_Player* player, Block_Buffer& buf){
	int8_t sync_type = 0;
	...
	switch(sync_type){
		case 1:{// 结算奖励
			...
			for(int i = 0; i < act_player_num; ++i){
				...
				//ACTIVE_CONTENT_MANAGER->active_content_listen_world_boss_end_info(role_id, damage, rank);
		...
		case 11:{
			...
			if(player && cost_glod > 0 ){
				...
				int cost_tp = CONFIG_CACHE_WORLD_BOSS->get_ability1_cost_type(gang_boss_act_icon_id);
				if(cost_tp == 1){
		...
		case 12:{
			...
			if(player && cost_glod > 0 ){
				Money_Sub_Type cost_type = GOLD_ONLY;
				int cost_tp = CONFIG_CACHE_WORLD_BOSS->get_ability2_cost_type(gang_boss_act_icon_id);
				if(cost_tp == 1){
					cost_type = BIND_GOLD_FIRST;
				}
		...
		case 13:{
			...
			if(player && cost_glod > 0 ){
				Money_Sub_Type cost_type = GOLD_ONLY;
				int cost_tp = CONFIG_CACHE_WORLD_BOSS->get_ability3_cost_type(gang_boss_act_icon_id);
				if(cost_tp == 1){
					cost_type = BIND_GOLD_FIRST;
				}
				int sub_result = player->pack_sub_money(Money_Sub_Info(Money_Sub_Type(cost_type), cost_glod, MONEY_SUB_WORLD_BOSS_POWER_UP));
...
int Gang_Boss_Active_Manager::inner_act_end(Block_Buffer& buf){
	role_id_t gang_id = 0;
	int result = 0;
	buf.read_int64(gang_id);
	buf.read_int32(result);
	GANG_MANAGER->set_boss_act_result(gang_id, result);
	act_open_ = false;
	return 0;
}
